...
### 2.2 LOAM Interface
- **功能**: 坐标系转换中间件。Point-LIO 输出的 `lidar_odom` 坐标系通常不符合 ROS 标准的 `odom -> base_footprint` 结构，该模块负责将其转换。
- **实现逻辑**: 订阅 Point-LIO 的里程计和点云，利用 `base_footprint -> front_mid360` 的静态 TF，将位姿和点云转换到以底盘为中心的坐标系下。
- **输入话题**: `cloud_registered`, `aft_mapped_to_init`
- **输出话题**:
    - `registered_scan`: 转换后的全局点云。
    - `lidar_odometry`: 转换后的标准里程计。

### 2.3 Sensor Scan Generation
- **功能**: 维护 TF 树的完整性，并处理云台运动补偿。
- **核心任务**:
    1. 发布 `odom -> base_footprint` 的动态变换。
    2. 发布 `gimbal_yaw -> gimbal_yaw_fake` 的辅助 TF。由于哨兵云台频繁旋转，直接使用云台坐标系会导致导航代价地图剧烈晃动，`gimbal_yaw_fake` 作为一个相对底盘静止的虚拟坐标系，解决了这一问题。
